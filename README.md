...
默认节奏偏保守（约每篇 5~10 秒，每 15 篇歇 2 分钟左右，一小时约 100 篇），以降低触发微信风控的概率。想更稳可以再放慢并分批：

```bash
wxmp --delay 10 --rest-every 10 --rest-seconds 300 crawl --biz xxx --max 30
```

注意 `--delay` 等是全局参数，要写在子命令前面。若出现“环境异常 / 验证码”，工具会自动停止本轮，建议等 1~2 小时后再继续（会自动增量续跑）。
...
- `--delay N`：请求间隔基准秒数（默认 6，实际乘以 0.8~1.6 随机抖动；历史列表翻页再乘 1.5；遇到“环境异常/操作频繁”就调大）
- `--rest-every N` / `--rest-seconds S`：批量下载时每连续下载 N 篇长休息 S 秒（默认 15 篇 / 120 秒，`--rest-every 0` 关闭）
